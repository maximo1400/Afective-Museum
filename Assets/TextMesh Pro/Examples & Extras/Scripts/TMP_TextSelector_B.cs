    ...
    public class TMP_TextSelector_B : MonoBehaviour, IPointerEnterHandler, IPointerExitHandler, IPointerClickHandler, IPointerUpHandler
    {
        ...
        void LateUpdate()
        {
            if (isHoveringObject)
            {
                ...
                int charIndex = TMP_TextUtilities.FindIntersectingCharacter(m_TextMeshPro, Mouse.current.position.ReadValue(), m_Camera, true);
                ...
                int wordIndex = TMP_TextUtilities.FindIntersectingWord(m_TextMeshPro, Mouse.current.position.ReadValue(), m_Camera);
