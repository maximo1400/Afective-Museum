...
namespace TMPro
{
    ...
    public class TMP_TextEventHandler : MonoBehaviour, IPointerEnterHandler, IPointerExitHandler
    {
        ...
        void LateUpdate()
        {
            if (TMP_TextUtilities.IsIntersectingRectTransform(m_TextComponent.rectTransform, Mouse.current.position.ReadValue(), m_Camera))
            {
                #region Nearest Character
                /*int charIndex = TMP_TextUtilities.FindNearestCharacterOnLine(m_TextComponent, Mouse.current.position.ReadValue(), 0, m_Camera, false);
                ...
                int charIndex = TMP_TextUtilities.FindIntersectingCharacter(m_TextComponent, Mouse.current.position.ReadValue(), m_Camera, true);
                if (charIndex != -1 && charIndex != m_lastCharIndex)
                {
                    m_lastCharIndex = charIndex;
                ...
                int wordIndex = TMP_TextUtilities.FindIntersectingWord(m_TextComponent, Mouse.current.position.ReadValue(), m_Camera);
                ...
                // Check if Mouse intersects any words and if so assign a random color to that word.
                int lineIndex = TMP_TextUtilities.FindIntersectingLine(m_TextComponent, Mouse.current.position.ReadValue(), m_Camera);
                ...
                int linkIndex = TMP_TextUtilities.FindIntersectingLink(m_TextComponent, Mouse.current.position.ReadValue(), m_Camera);
